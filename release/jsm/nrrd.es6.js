...
const assert = function ( c, msg ) {
  if (!c) {
    console.warn( "Assertion failed in " + this.caller.name + ": " + msg );
  }
}
 ...
 * RB PATCH START 
 * 
 * new function to parse only the header of an NRRD file (copied from nrrd.parse)
 */
export function parseHeader(buffer) {
    ...
    if (dataStart === undefined) {
        ...
        // RB: do not store buffer, but store dataStart instead as 'byteOffset'
        ret.byteOffset = dataStart;
    }
    ...
    // RB: ret.data needs to be set otherwise checkNRRD fails
    ret.data = 'Header only';
    checkNRRD(ret);
    
    // RB: this is where the DATA parsing part was cut
...
export function parse(buffer) {
    ...
    if ('dataFile' in ret) {
        ...
        case 'gzip': // AP: experimental
            ret.data = parseNRRDRawData(pako.inflate(ret.buffer), ret.type, ret.sizes, {
                endian: ret.endian, blockSize: ret.blockSize
            });
            break;
        default:
...
export function parseNRRDTextData(buffer, type, sizes) {
