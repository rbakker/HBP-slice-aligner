...
/*
this.autoCompress = () => {
  if (this.data.byteLength < this.autoCompressThreshold) {
    this.autoCompress = () => { return this; } // disable autoCompress
  } else {
    this.autoCompress();
  }
}
...
ndArray_class.prototype.autoCompressDelay = 10000; // wait this many milliseconds before autocompressing data, use 0 to disable
ndArray_class.prototype.autoCompressThreshold = 1000000; // only compress data larger than this many bytes 
/*
 * Autocompression DISABLED
 * 
if (window.Worker) {
  ndArray_class.compressor = {
    worker: new Worker("compressor.js.php"),
    jobHandler: {},
    lastJob: 0
  }
  ndArray_class.compressor.worker.onmessage = (e) => { 
    const compressor = ndArray_class.compressor;
    const jobId = e.data.id;
    compressor.jobHandler[jobId](e.data.byteArray_gz);
    delete compressor.jobHandler[jobId];
  };
}
*/

ndArray_class.prototype.autoCompress = function() {
/*
 * Autocompression DISABLED
 * 
  if ( this.autoCompressDelay > 0 ) {
    if (this.autoCompressQueue !== undefined) clearTimeout(this.autoCompressQueue);
    this.autoCompressQueue = setTimeout( () => {
      if (this.data) {
        let compressor = ndArray_class.compressor;
        if (compressor) {
          const jobId = compressor.lastJob+1;
          compressor.lastJob = jobId;
          compressor.jobHandler[jobId] = (byteArray_gz) => {
            this.data_gz = [this.data.constructor,byteArray_gz];
            this.data = undefined;
          }
          compressor.worker.postMessage({
            id:jobId,
            byteArray:new Uint8Array(this.data.buffer,this.data.byteOffset,this.data.byteLength)
          });
        }
      }
    }, this.autoCompressDelay);
  }
*/
  return this;
}

ndArray_class.prototype.decompress = function() {
/*
 * Autocompression DISABLED
 * 
  if (this.data_gz) {
    this.data = new this.data_gz[0](window.pako.inflate(this.data_gz[1]).buffer);
    this.data_gz = undefined;
  }
*/
  return this;
}

ndArray_class.prototype.getChunks = function() {
  this.autoCompress() // resets the autocompress timer
  return this.chunks || this.decompress().chunks;
}
